...
export default function AppLoader({
  ...
  const content = (
    <div className={`${card ? "app-card p-4 text-center h-100" : "text-center"} d-flex flex-column align-items-center justify-content-center`}>
      <div
        ...
        role="status"
      />
      {label ? <div className="mt-3 small text-secondary">{label}</div> : null}
    </div>
  );
