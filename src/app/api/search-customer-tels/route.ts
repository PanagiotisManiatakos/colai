...
export async function GET(req: Request) {
    ...
    if (!customer_GID && !customerAMKA) {
        return NextResponse.json({ ok: false, message: "customer_GID or customerAMKA required" }, { status: 400 });
    }

    const upstream = new URL(`${process.env.AMSA_API_BASE_URL}/api/search-customer-tels`);
